...
%define BASE_CONFIGURE_ARGS $(echo "--prefix=%{_sysconfdir}/nginx --sbin-path=%{_sbindir}/nginx --modules-path=%{_libdir}/nginx/modules --conf-path=%{_sysconfdir}/nginx/nginx.conf --error-log-path=%{_localstatedir}/log/nginx/error.log --http-log-path=%{_localstatedir}/log/nginx/access.log --pid-path=%{_localstatedir}/run/nginx.pid --lock-path=%{_localstatedir}/run/nginx.lock --http-client-body-temp-path=%{_localstatedir}/cache/nginx/client_temp --http-proxy-temp-path=%{_localstatedir}/cache/nginx/proxy_temp --http-fastcgi-temp-path=%{_localstatedir}/cache/nginx/fastcgi_temp --http-uwsgi-temp-path=%{_localstatedir}/cache/nginx/uwsgi_temp --http-scgi-temp-path=%{_localstatedir}/cache/nginx/scgi_temp --user=%{nginx_user} --group=%{nginx_group} --with-compat --with-file-aio --with-threads --with-http_addition_module --with-http_auth_request_module --with-http_dav_module --with-http_flv_module --with-http_gunzip_module --with-http_gzip_static_module --with-http_mp4_module --with-http_random_index_module --with-http_realip_module --with-http_secure_link_module --with-http_slice_module --add-module=ModSecurity-nginx-1.0.4 --add-module=zstd-nginx-module-0.1.1 --with-http_ssl_module --with-http_stub_status_module --with-http_sub_module --with-http_v2_module $( if [ 0%{?rhel} -eq 7 ] || [ 0%{?suse_version} -eq 1315 ]; then continue; else echo "--with-http_v3_module"; fi; ) --with-mail --with-mail_ssl_module --with-stream --with-stream_realip_module --with-stream_ssl_module --with-stream_ssl_preread_module")
...
Source11: https://github.com/openssl/openssl/releases/download/openssl-3.5.6/openssl-3.5.8.tar.gz
...
BuildRequires: modsecurity
Requires: modsecurity%{?_isa}
...
%build
export PKG_CONFIG_PATH=/opt/modsecurity/lib64/pkgconfig${PKG_CONFIG_PATH:+:$PKG_CONFIG_PATH}
...
sed -i \
    ...
    ModSecurity-nginx-1.0.4/config

test -f /opt/modsecurity/include/modsecurity/modsecurity.h
test -f /opt/modsecurity/lib64/libmodsecurity.so
...
%changelog
* Tue Apr 14 2026 Nginx Packaging <nginx-packaging@f5.com> - 1.30.0-1%{?dist}.ngx
- 1.30.0-1

* Tue Apr  7 2026 Nginx Packaging <nginx-packaging@f5.com> - 1.29.8-1%{?dist}.ngx
- 1.29.8-1

* Tue Mar 24 2026 Nginx Packaging <nginx-packaging@f5.com> - 1.29.7-1%{?dist}.ngx
- 1.29.7-1
...
* Tue Oct  7 2025 Nginx Packaging <nginx-packaging@f5.com> - 1.29.2-1%{?dist}.ngx
- 1.29.2-1

* Wed Aug 13 2025 Nginx Packaging <nginx-packaging@f5.com> - 1.29.1-1%{?dist}.ngx
- 1.29.1-1
